...
export async function logoutUser(refreshToken: string): Promise<void> {
  ...
  await prisma.refreshToken.deleteMany({ where: { token: hashOpaqueToken(refreshToken) } });
}
...
export async function resetPassword(token: string, newPassword: string): Promise<void> {
  ...
  await prisma.passwordResetToken.update({ where: { token: tokenHash }, data: { used: true } });
