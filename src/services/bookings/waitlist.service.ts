...
export async function joinWaitlist(serviceId: string, seekerId: string) {
  ...
  const ongoingCount = await prisma.booking.count({ where: { serviceId, status: "ONGOING" } });
  const waitingCount = await prisma.queue.count({ where: { serviceId, status: "WAITING" } });
