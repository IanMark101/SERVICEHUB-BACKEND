...
import { adminResolveCancellationRequest } from "./services/cancellation.service";
...
async function runE2ETests() {
  ...
  try {
    ...
    await prisma.transaction.deleteMany({ where: { description: { contains: "cancelled" } } });
    await prisma.transaction.deleteMany({ where: { description: { contains: "confirmed" } } });
    await prisma.review.deleteMany({ where: { text: { contains: "E2E" } } });
    await prisma.report.deleteMany({ where: { description: { contains: "Dispute" } } });
    await prisma.cancellationRequest.deleteMany({ where: { reason: { contains: "E2E" } } });
    await prisma.queue.deleteMany({ where: { seeker: { email: { startsWith: "e2e-test-" } } } });
    await prisma.booking.deleteMany({ where: { seeker: { email: { startsWith: "e2e-test-" } } } });
    await prisma.offer.deleteMany({ where: { provider: { email: { startsWith: "e2e-test-" } } } });
    await prisma.serviceRequest.deleteMany({ where: { seeker: { email: { startsWith: "e2e-test-" } } } });
    await prisma.service.deleteMany({ where: { provider: { email: { startsWith: "e2e-test-" } } } });
    await prisma.user.deleteMany({ where: { email: { startsWith: "e2e-test-" } } });
    ...
    const requestSelf = await prisma.serviceRequest.create({
      data: {
        seekerId: provider.id,
        categoryId: category.id,
    ...
    const directReq = await createDirectRequest({
      seekerId: seeker.id,
      providerId: provider.id,
      serviceId: service.id,
      agreedPrice: 150.00,
    ...
    const acceptedBooking = await respondToDirectBookingService(directReq.id, provider.id, true);
    assert((acceptedBooking as any).status === "ACCEPTED" && (acceptedBooking as any).paymentMethod === "On-site Cash", "Cash direct request accepted -> status ACCEPTED, paymentMethod On-site Cash");
    ...
    const offer1 = await submitOffer(provider.id, {
    ...
    // Accept this offer
    await acceptOffer(offer1.id, seeker.id);

    // Verify request status and offer status updates
    const requestUpdated = await prisma.serviceRequest.findUnique({ where: { id: request.id } });
    const offer1Updated = await prisma.offer.findUnique({ where: { id: offer1.id } });
    ...
    const queueRes1 = await addToQueue({
      serviceId: service.id,
      seekerId: seeker2.id,
    ...
    const cancelRes = await cancelQueueEntry(queueRes1.queueEntry.id, seeker2.id);
    ...
    const queueRes2 = await addToQueue({
      serviceId: service.id,
      seekerId: seeker.id,
      paymentId: "pay_intent_immediate"
    });
    
    const immediateBooking = await prisma.booking.findUnique({
    ...
    const seeker4 = await prisma.user.create({
      data: { name: "Seeker 4", email: "e2e-test-seeker4@example.com", passwordHash: "h", phone: "0", location: "C" }
    });

    const q3 = await addToQueue({ serviceId: service.id, seekerId: seeker3.id, paymentId: "p3" });
    const q4 = await addToQueue({ serviceId: service.id, seekerId: seeker4.id, paymentId: "p4" });

    // Cancel seeker 3 (position 2)
    await cancelQueueEntry(q3.queueEntry.id, seeker3.id);

    // Verify seeker 4 (position 3) drops to position 2 in both Queue and Booking tables
    const q4Updated = await prisma.queue.findUnique({ where: { id: q4.queueEntry.id } });
    const b4Updated = await prisma.booking.findUnique({ where: { id: q4.queueEntry.bookingId } });
    ...
    await confirmCompletionService(immediateBooking!.id, seeker.id);

    const providerTrustUpdated = await prisma.user.findUnique({ where: { id: provider.id }, select: { trustScore: true } });
    const seekerTrustUpdated = await prisma.user.findUnique({ where: { id: seeker.id }, select: { trustScore: true } });
