...
function validSignature(rawBody: Buffer, header: string | undefined): boolean {
  ...
  return [parts.te, parts.li].some((signature) => {
    if (!signature || !/^[a-f0-9]{64}$/i.test(signature) || signature.length !== expected.length) return false;
...
export async function receivePaymongoWebhook(req: Request, res: Response, next: NextFunction) {
  ...
  try {
    ...
    if (typeof paymentIntentId !== "string") return res.status(200).json({ success: true, ignored: true });

    const intent = await getPaymentIntent(paymentIntentId);
    const metadata = intent.metadata;
    const expectedAmount = Number(metadata.servicehub_expected_amount);
    const methodMap: Record<string, "GCash" | "Maya" | "Card"> = { gcash: "GCash", paymaya: "Maya", card: "Card" };
