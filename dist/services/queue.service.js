...
async function recalculateQueue(serviceId) {
    const activeEntries = await prisma_1.prisma.queue.findMany({
...
async function notifyWaitlist(serviceId) {
    const service = await prisma_1.prisma.service.findUnique({
        where: { id: serviceId },
    ...
    if (!service)
        return;
    const currentQueueSize = await prisma_1.prisma.queue.count({
        where: { serviceId, status: { in: ["WAITING", "SERVING"] } },
    });
    if (currentQueueSize < service.queueLimit) {
        // Notify the first person on the waitlist
        const firstWaiting = await prisma_1.prisma.queueNotify.findFirst({
            where: { serviceId },
        ...
        if (firstWaiting) {
            ...
            await prisma_1.prisma.queueNotify.delete({ where: { id: firstWaiting.id } });
        }
    }
}
