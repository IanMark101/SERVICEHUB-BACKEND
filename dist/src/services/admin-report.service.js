...
function activityLink(report, userId, tab = "all") {
    return userId === report.booking.providerId
        ? `/provider/provider-activity?tab=${tab}`
        : `/seeker/seeker-activity?tab=${tab}`;
}
export async function resolveAdminReport(reportId, adminId, action, adminNotes) {
    ...
    const report = await prisma.$transaction(async (tx) => {
        ...
        if (claimed.count !== 1) {
            const error = new Error("Report has already been resolved");
            error.status = 409;
            throw error;
        }
    ...
    if (action === "suspend" || action === "ban") {
        await disconnectUserSockets(report.reportedUserId, action === "ban" ? "Account permanently banned" : "Account temporarily suspended");
    }
...
async function resolveRefundReport(reportId, adminId, adminNotes) {
    ...
    if (report.status !== "PENDING") {
        const error = new Error("Report is already being reviewed or resolved");
        error.status = 409;
        throw error;
    }
    if (!report.booking.queue || !["PAID_HELD", "FROZEN_HELD"].includes(report.booking.paymentStatus)) {
        const error = new Error("Only a held online payment can be refunded");
        error.status = 422;
        throw error;
    }
    ...
    if (claimed.count !== 1) {
        const error = new Error("Report is already being reviewed");
        error.status = 409;
        throw error;
    }
    let refund;
    try {
        refund = await refundBookingPayment(report.bookingId, adminId, `Administrator-approved refund: ${adminNotes}`);
    }
    catch (cause) {
