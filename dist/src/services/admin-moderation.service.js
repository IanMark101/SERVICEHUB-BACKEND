...
export async function reviewServiceListing(serviceId, adminId, approve, adminNotes) {
    const result = await prisma.$transaction(async (tx) => {
        ...
        if (service.status !== "PENDING_REVIEW") {
            const error = new Error("Service listing has already been reviewed");
            error.status = 409;
            throw error;
        }
        if (approve && service.provider.verificationStatus !== "APPROVED") {
            const error = new Error("Provider must be a Verified Resident before approval");
            error.status = 422;
            throw error;
        }
...
export async function resolveCategory(suggestionId, adminId, approve, adminNotes) {
    const suggestion = await prisma.$transaction(async (tx) => {
        const current = await tx.categorySuggested.findUnique({ where: { id: suggestionId } });
        if (!current) {
            const error = new Error("Category suggestion not found");
            error.status = 404;
            throw error;
        }
        if (current.status !== "PENDING") {
            const error = new Error("Category suggestion has already been reviewed");
            error.status = 409;
            throw error;
        }
        ...
        if (approve) {
            ...
            if (duplicate) {
                const error = new Error("An equivalent category already exists");
                error.status = 409;
                throw error;
            }
...
//# sourceMappingURL=admin-moderation.service.js.map