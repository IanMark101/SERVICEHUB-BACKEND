...
export async function requestCancellation(bookingId, seekerId, reason) {
    const booking = await prisma.booking.findUnique({
        where: { id: bookingId }
    });
...
export async function respondToCancellationRequest(requestId, providerId, approve, providerNote) {
...
export async function escalateCancellationRequest(requestId, seekerId) {
    const cancelReq = await prisma.cancellationRequest.findUnique({
        where: { id: requestId },
        include: { booking: true }
    });
