...
export async function confirmOnlineBooking(req, res, next) {
    try {
        ...
        const isBoundToBooking = intent.status === "succeeded" &&
            intent.currency === "PHP" &&
...
export async function hideBooking(req, res, next) {
    try {
        const user = req.user;
        const { id } = req.params;
        const { hideBookingService } = await import("../services/bookings.service");
        const result = await hideBookingService(id, user.id);
