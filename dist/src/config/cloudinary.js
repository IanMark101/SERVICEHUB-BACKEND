...
const hasSeparateKeys = Boolean(process.env.CLOUDINARY_CLOUD_NAME &&
 ...
 */
export async function uploadImageToCloudinary(fileData, options = {}) {
    ...
    if (!isConfigured) {
        const error = new Error('Image uploads are unavailable because Cloudinary is not configured.');
        error.status = 503;
        throw error;
    }
    try {
...
export async function uploadPrivateVerificationImage(fileData, userId) {
    if (!isConfigured) {
        const error = new Error('Verification uploads are unavailable because Cloudinary is not configured.');
        error.status = 503;
        throw error;
    }
    const result = await cloudinary.uploader.upload(fileData, {
...
export function getPrivateVerificationUrl(storageKey) {
...
export async function deletePrivateVerificationImage(storageKey) {
    if (!isConfigured)
        return;
